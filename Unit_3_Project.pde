...
public void setup() {
  ...
  ShootHere = new Vector2D(0,0);
  
  for(int i = 0; i < numbullets; i++)
  {
   bullets[i] = new Bullet(width, height, (ShootHere.add(mouseX,mouseY)), 1, bulletnum);
  }
...
void draw() {
  ...
  for(int i = 0; i < numbullets; i++)
  {
   bullets[i].display();
   bullets[i].move();
   boolean bang = bullets[i].collision();
   if(bang){
     stop();
   }
  }//
...
void drawRoute() {
  ...
  if (route.size() > 1) {
    ...
    for (int i = 1; i < route.size(); i++) {
      ep = route.get(i);
      line((float)sp.x, (float)sp.y, (float)ep.x, (float)ep.y);
      sp = ep;
    }
  }
}
 ...
 void mousePressed()
  {
    ShootHere = new Vector2D(mouseX, mouseY);
    Vector2D dir;
    dir = ShootHere.sub(a.pos());
    dir.normalize();
    
   bulletnum = (bulletnum + 1) % numbullets;
   //float x = tan((mouseX - turretX)/(turretY - mouseY));
   bullets[bulletnum] = new Bullet(turretX, turretY, dir, 3, bulletnum);
  }